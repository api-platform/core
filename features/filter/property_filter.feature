Feature: Set properties to include
  ...
  Scenario: Test properties filter
    ...
    And I send a "GET" request to "/dummies/1?properties[]=name&properties[]=alias&properties[]=relatedDummy&properties[]=name_converted"
  ...
  Scenario: Test relation embedding
    When I add "Accept" header equal to "application/ld+json"
    And I send a "GET" request to "/dummies/1?properties[]=name&properties[]=alias&properties[relatedDummy][]=name"
    And the JSON node "name" should be equal to "Dummy #1"
    And the JSON node "alias" should be equal to "Alias #0"
