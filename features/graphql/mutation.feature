Feature: GraphQL mutation support
  ...
  Scenario: Trigger an error trying to delete item of different resource
    ...
    And the response should be in JSON
    And the header "Content-Type" should be equal to "application/json"
    And the JSON node "errors[0].message" should match '/^Item "\/dummies\/1" did not match expected type "ApiPlatform\\Core\\Tests\\Fixtures\\TestBundle\\(Document|Entity)\\Foo".$/'
