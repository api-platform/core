Feature: GraphQL mutation support
  ...
  Scenario: Introspect types
    ...
    And the JSON node "data.__type.fields[0].type.name" should match "/^delete[a-zA-Z]+Payload$/"
    ...
    And the JSON node "data.__type.fields[0].args[0].type.name" should match "/^delete[a-zA-Z]+Input$/"
