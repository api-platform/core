...
final class ParameterProvider implements ProviderInterface
{
    ...
    public function provide(Operation $operation, array $uriVariables = [], array $context = []): object|array|null
    {
        ...
        $links = $operation instanceof HttpOperation ? $operation->getUriVariables() ?? [] : [];
        ...
        foreach ($links as $key => $uriVariable) {
            ...
            $context['operation'] = $operation = $this->callParameterProvider($operation, $uriVariable, $values, $context);
        }

        if (\count($links) && $operation instanceof HttpOperation) {
            $operation = $operation->withUriVariables($links);
        }
