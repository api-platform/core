...
class OrderFilterTest extends TestCase
{
    ...
    public function testQueryParameterWithCamelCaseProperty(): void
    {
        ...
        $this->assertSame(\DB::getQueryLog()[1]['query'], 'select * from "active_books" order by "is_active" asc limit 30 offset 0');
        DB::flushQueryLog();
        $response = $this->get('/api/active_books?sort[isActive]=desc', ['Accept' => ['application/ld+json']]);
        $response->assertStatus(200);
        $this->assertSame(DB::getQueryLog()[1]['query'], 'select * from "active_books" order by "is_active" desc limit 30 offset 0');
    }
}
