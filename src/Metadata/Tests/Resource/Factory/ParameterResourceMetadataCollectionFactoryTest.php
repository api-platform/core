...
class ParameterResourceMetadataCollectionFactoryTest extends TestCase
{
    ...
    public function testQueryParameterFromPropertyAttributeUsesPropertyNameAsDefaultKey(): void
    {
        $nameCollection = $this->createStub(PropertyNameCollectionFactoryInterface::class);
        $nameCollection->method('create')->willReturn(new PropertyNameCollection(['id', 'name', 'isActive']));
        ...
        $resourceMetadataCollection = $parameterFactory->create(ParameterOnPropertiesWithDefaultKey::class);
        $operation = $resourceMetadataCollection->getOperation(forceCollection: true);
        $parameters = $operation->getParameters();
        ...
        $this->assertTrue($parameters->has('name'));
        $nameParam = $parameters->get('name', QueryParameter::class);
        $this->assertInstanceOf(QueryParameter::class, $nameParam);
        $this->assertSame('name', $nameParam->getProperty());
        $this->assertSame($nameParam->getKey(), $nameParam->getProperty());
        $this->assertSame('Search by name', $nameParam->getDescription());

        $this->assertTrue($parameters->has('isActive'));
        $isActiveParam = $parameters->get('isActive', QueryParameter::class);
        $this->assertInstanceOf(QueryParameter::class, $isActiveParam);
        $this->assertSame('isActive', $isActiveParam->getProperty());
        $this->assertSame($isActiveParam->getKey(), $isActiveParam->getProperty());
        $this->assertSame('Filter by active status', $isActiveParam->getDescription());
    }
...
#[ApiResource]
class ParameterOnPropertiesWithDefaultKey
{
    #[QueryParameter(description: 'Search by name')]
    public string $name = '';

    #[QueryParameter(description: 'Filter by active status')]
    public bool $isActive = true;
}
