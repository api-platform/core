...
SQL;
    ...
    public function testCompositeIdentifiers(): void
    {
        ...
        $expected = <<<SQL
SELECT o
FROM ApiPlatform\Doctrine\Orm\Tests\Fixtures\Entity\CompositeRelation o
INNER JOIN o.compositeItem item
INNER JOIN o.compositeLabel label
WHERE o.item IN(
    SELECT IDENTITY(o_2.item) FROM ApiPlatform\Doctrine\Orm\Tests\Fixtures\Entity\CompositeRelation o_2
    INNER JOIN o_2.compositeItem item_2
    INNER JOIN o_2.compositeLabel label_2
    WHERE item_2.field1 = :foo
) AND o.label IN(
    SELECT IDENTITY(o_2.label) FROM ApiPlatform\Doctrine\Orm\Tests\Fixtures\Entity\CompositeRelation o_2
    INNER JOIN o_2.compositeItem item_2
    INNER JOIN o_2.compositeLabel label_2
    WHERE item_2.field1 = :foo
)
...
LEFT JOIN ApiPlatform\Doctrine\Orm\Tests\Fixtures\Entity\DummyCar car WITH car.id = o.car
WHERE o.item IN(
    SELECT IDENTITY(o_2.item) FROM ApiPlatform\Doctrine\Orm\Tests\Fixtures\Entity\CompositeRelation o_2
    INNER JOIN o_2.compositeItem item_2
    INNER JOIN o_2.compositeLabel label_2
...
LEFT JOIN o.colors colors
WHERE o.id IN(
  SELECT IDENTITY(o_2.id) FROM ApiPlatform\Doctrine\Orm\Tests\Fixtures\Entity\DummyCar o_2
  LEFT JOIN o_2.colors colors_2
  WHERE o_2.colors = :foo
)
SQL;
