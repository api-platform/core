...
final class ContainerParameterResourceMetadataCollectionFactoryTest extends TestCase
{
    ...
    {
        $collection = new ResourceMetadataCollection('Foo', [
            (new ApiResource())->withRoutePrefix('/%app.prefix%/v1'),
        ]);

        $factory = $this->createFactory($collection, ['app.prefix' => 'api']);

        $this->assertSame('/api/v1', $factory->create('Foo')[0]->getRoutePrefix());
    }
    ...
    public function testItResolvesGraphQlOperationSecurityExpressionParameter(): void
    {
        $collection = new ResourceMetadataCollection('Foo', [
            (new ApiResource())->withGraphQlOperations([
                'query' => (new Query())->withSecurity('%app.security%'),
            ]),
        ]);

        $factory = $this->createFactory($collection, ['app.security' => 'is_granted("ROLE_ADMIN")']);

        $this->assertSame('is_granted("ROLE_ADMIN")', $factory->create('Foo')[0]->getGraphQlOperations()['query']->getSecurity());
    }
}
