...
final class SwaggerUiProcessor implements ProcessorInterface
{
    ...
    private function resolveCspNonce(?Request $request): ?string
    {
        ...
        // Rendering through Twig lets it resolve runtime functions, which a raw callable cannot (#8560).
        if ($this->twig->getFunction('csp_nonce')) {
            $nonce = $this->twig->createTemplate("{{ csp_nonce('script')|raw }}")->render();
