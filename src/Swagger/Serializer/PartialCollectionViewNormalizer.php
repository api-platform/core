...
/**
 * Adds a view key to the result of a paginated Swagger collection.
 *
...
final class PartialCollectionViewNormalizer implements NormalizerInterface, SerializerAwareInterface
{
    ...
    public function normalize($object, $format = null, array $context = [])
    {
        ...
        if ($format  === self::FORMAT) {
            ...
            if ($paginated) {
                $data['view']['first'] = $this->getId($parts, $parameters, 1.);
                $data['view']['last'] = $this->getId($parts, $parameters, $lastPage);

                if (1. !== $currentPage) {
                    $data['view']['previous'] = $this->getId($parts, $parameters, $currentPage - 1.);
                }

                if ($currentPage !== $lastPage) {
                    $data['view']['next'] = $this->getId($parts, $parameters, $currentPage + 1.);
                }
            }
        }
