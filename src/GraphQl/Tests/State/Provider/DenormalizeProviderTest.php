...
class DenormalizeProviderTest extends TestCase
{
    public function testProvide(): void
    {
        ...
        $serializerContextBuilder->expects($this->once())->method('create')->with($operation->getApiClass(), $operation, $context, false)->willReturn($serializerContext);
    ...
    public function testProvideWithObjectToPopulate(): void
    {
        ...
        $context = ['args' => ['input' => ['test']]];
        $operation = new Mutation(class: \stdClass::class);
        $serializerContext = ['resource_class' => $operation->getApiClass(), 'object_to_populate' => $objectToPopulate];
        $decorated = $this->createMock(ProviderInterface::class);
        $decorated->expects($this->once())->method('provide')->willReturn($objectToPopulate);
        $denormalizer = $this->createMock(DenormalizerInterface::class);
        $serializerContextBuilder = $this->createMock(SerializerContextBuilderInterface::class);
    ...
    public function testProvideNotCalledWithQuery(): void
    {
        ...
        $denormalizer = $this->createMock(DenormalizerInterface::class);
        $serializerContextBuilder = $this->createMock(SerializerContextBuilderInterface::class);
        $serializerContextBuilder->expects($this->never())->method('create')->with($operation->getApiClass(), $operation, $context, false)->willReturn($serializerContext);
