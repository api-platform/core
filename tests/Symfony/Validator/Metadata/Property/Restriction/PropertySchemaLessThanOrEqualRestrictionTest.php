...
final class PropertySchemaLessThanOrEqualRestrictionTest extends TestCase
{
    ...
    {
        yield 'supported int/float with union types' => [new LessThanOrEqual(['value' => 10]), (new ApiProperty())->withBuiltinTypes(Type::union(Type::int(), Type::float())), true];
        yield 'supported int' => [new LessThanOrEqual(['value' => 10]), (new ApiProperty())->withBuiltinTypes(Type::int()), true];
        yield 'supported float' => [new LessThanOrEqual(['value' => 10.99]), (new ApiProperty())->withBuiltinTypes(Type::float()), true];
        yield 'supported negative or zero' => [new NegativeOrZero(), (new ApiProperty())->withBuiltinTypes(Type::int()), true];
        yield 'not supported negative' => [new Negative(), (new ApiProperty())->withBuiltinTypes(Type::int()), false];
        yield 'not supported property path' => [new LessThanOrEqual(['propertyPath' => 'greaterThanMe']), (new ApiProperty())->withBuiltinTypes(Type::int()), false];
    }
    ...
    {
        self::assertEquals(['maximum' => 10], $this->propertySchemaLessThanOrEqualRestriction->create(new LessThanOrEqual(['value' => 10]), (new ApiProperty())->withBuiltinTypes(Type::int())));
    }
}
