...
class IriConverterTest extends TestCase
{
    ...
    public function testGetIriFromItemOperationWithCanonicalUriTemplate(): void
    {
        $item = new Dummy();
        $item->setId(1);

        // e.g. a PATCH operation with a custom URI template, whose IRI must point to the canonical operation
        $operation = (new Patch())->withName('patch_custom')->withCanonicalUriTemplate('/dummies/{id}{._format}');
        ...
        $iriConverter = $this->getMockedIriConverter(
            $router,
    ...
    public function testGetIriFromItemOperationWithAnUnresolvableCanonicalUriTemplate(): void
    {
        $item = new Dummy();
        $item->setId(1);

        $operation = (new Patch())->withName('patch_custom')->withCanonicalUriTemplate('/does-not-exist/{id}');
