...
use ApiPlatform\Tests\Fixtures\TestBundle\ApiResource\PostInputDtoQueryParameterIsolation\PostInputDtoQueryParameterIsolation;
...
final class HttpQueryMethodInputDtoTest extends ApiTestCase
{
    ...
    {
        return [QueryInputDummy::class, PostInputDtoQueryParameterIsolation::class];
    }
    ...
    public function testInputDtoQueryParametersAreNotAddedToPostOperations(): void
    {
        $response = self::createClient()->request('GET', '/docs', [
            'headers' => ['Accept' => 'application/vnd.openapi+json'],
        ]);
        ...
        $this->assertResponseIsSuccessful();
        $post = $response->toArray()['paths']['/post_input_dto_query_parameter_isolation']['post'];

        foreach ($post['parameters'] ?? [] as $parameter) {
            $this->assertNotSame('name', $parameter['name'] ?? null);
        }
    }
}
